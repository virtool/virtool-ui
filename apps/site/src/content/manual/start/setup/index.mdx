...
Passing the `--no-setup` argument when running Virtool will skip the setup process and use default configuration values or values from [manual configuration sources](/docs/start/configuration).
...
1. After installing Virtool, issue the following command in the Virtool install directory
   ...
   Use `./run --help` to see an overview of additional command line arguments Virtool can accept or read the [configuration documentation](/docs/start/configuration).
...
The MongoDB connection string is stored in plain text in the application configuration file. [Configure Virtool using environmental variables](/docs/start/configuration) to keep your connection string safe.
