...
genuinely client-only shapes; `references/`, `indexes/`, `jobs/`,
`labels/`, `users/` and `account/` have no `types.ts` left at all, because
every shape they had was a wire shape). A
