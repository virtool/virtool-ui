...
- `packages/` — shared, framework-agnostic libraries published as workspace
  ...
  - `@virtool/workflow` — the workflow runtime every executor runs on: the
    ...
    `buildContext` seam, and the job lifecycle loop that claims, heartbeats
    and reports over the jobs API. It knows nothing about storage — see the
    section below. It is the only place in the repo that spawns a process,
    and so the only one depending on `execa`.
...
`virtool/workflow/`, and it knows nothing about object storage.
