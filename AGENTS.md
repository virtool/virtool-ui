...
- `packages/` — shared, framework-agnostic libraries published as workspace
  ...
  - `@virtool/workflow` — the workflow runtime every executor runs on: the
    ...
    and the file layer — streaming transfer, gzip, tar and cache-key
    derivation. It takes a `StorageBackend` as an argument and knows nothing
    about HTTP, job claiming, or a database — see the section below.
...
subprocesses, job claiming, or a database — `runWorkflow` **returns** an outcome and never touches the
network, `process.exit`, or a signal handler.

Object storage is the one exception, and it is reached the way `db` is on
the server side: a `StorageBackend` is **passed in as an argument**, never
constructed here and never a module-level singleton. See the file layer
below.
