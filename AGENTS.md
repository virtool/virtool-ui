...
`sweep_blast` is the only body a user watches in real time — it advances
the NuVs BLAST searches somebody has just clicked for, which is why it
alone runs on a thirty-second period. Its body is one call to
`sweepBlasts` (`@virtool/data/blast/data`), over the client in
