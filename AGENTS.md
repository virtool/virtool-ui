...
is a raw route for the same reason: Prometheus speaks plain HTTP, not the
generated RPC client. It enforces its own floor too — a bearer token compared
against `VT_METRICS_TOKEN`, with an unset token reporting 404 rather than
serving openly.
